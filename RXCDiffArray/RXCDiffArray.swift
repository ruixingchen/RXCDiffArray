...
public final class RXCDiffArray<SectionContainer: RangeReplaceableCollection, RowElement> {
    ...
    public typealias Difference = RDADifference<SectionContainer.Element, RowElement>
...
extension RXCDiffArray where SectionContainer.Index==Int {
    ...
    public func element(at indexPath:IndexPath)->RowElement? {
        ...
        guard let rowElement = element as? RowElement else {
            print("获取的RowElement类型不正确:\(element)")
            return nil
        }
        return rowElement
    }
...
extension RXCDiffArray where SectionContainer.Index==Int {
    ...
    @discardableResult
    public func addSection(_ newElement: __owned SectionElement, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.addSection(contentsOf: CollectionOfOne(newElement), userInfo: userInfo)
    }

    @discardableResult
    public func addSection<S: Sequence>(contentsOf newElements: __owned S, userInfo:[AnyHashable:Any]?=nil)->Difference where S.Element==SectionElement {
        let array = [S.Element].init(newElements)
        return self.replaceSection(self.contentCollection.endIndex..<self.contentCollection.endIndex, with: array, userInfo: userInfo)
    }

    @discardableResult
    public func insertSection(_ newElement: __owned SectionElement, at i: SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.insertSection(contentsOf: CollectionOfOne(newElement), at: i, userInfo: userInfo)
    }

    @discardableResult
    public func insertSection<C>(contentsOf newElements: __owned C, at i: SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where C : Collection, C.Element==SectionElement {
        return self.replaceSection(i..<i, with: newElements, userInfo: userInfo)
    }

    //MARK: - Section 修改

    @discardableResult
    public func replaceSection(at position: SectionIndex, with newElement: SectionElement, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.replaceSection(position..<self.contentCollection.index(after: position), with: CollectionOfOne(newElement), userInfo: userInfo)
    }

    @discardableResult
    public func replaceSection<C:Collection, R:RangeExpression>(_ subrange: R, with newElements: __owned C, userInfo:[AnyHashable:Any]?=nil)->Difference where C.Element==RXCDiffArray.Element, R.Bound==SectionContainer.Index {
    ...
    public func removeEmptySection() {

    }

    @discardableResult
    public func removeSection(at position: SectionIndex, userInfo:[AnyHashable:Any]?=nil) -> Difference {
        return self.replaceSection(position..<self.contentCollection.index(after: position), with: EmptyCollection(), userInfo: userInfo)
    }

    @discardableResult
    public func removeSection(_ bounds: Range<SectionContainer.Index>, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.replaceSection(bounds, with: EmptyCollection(), userInfo: userInfo)
    }

    ///删除所有符合条件的一维数据
    @discardableResult
    public func removeAllSection(userInfo:[AnyHashable:Any]?=nil, where shouldBeRemoved: (SectionContainer.Element) -> Bool)->Difference {
    ...
    ///移动Section
    @discardableResult
    public func moveSection(from position:SectionIndex, to toPosition:SectionIndex,userInfo:[AnyHashable:Any]?=nil)->Difference {
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
...
//这里要求Int是为了兼容RDADifference
extension RXCDiffArray where Element: SectionElementProtocol, SectionIndex==Int {

    //MARK: - 二维 新增

    @discardableResult
    public func addRow(_ newElement: __owned RowElement, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.addRow(contentsOf: CollectionOfOne(newElement), in: s, userInfo: userInfo)
    }

    @discardableResult
    public func addRow<S>(contentsOf newElements: __owned S, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where S : Collection, S.Element==RowElement {
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
    ...
    @discardableResult
    public func insertRow(_ newElement: __owned RowElement, at i: RowIndex, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference {
        return self.insertRow(contentsOf: CollectionOfOne(newElement), at: i, in: s, userInfo: userInfo)
    }

    @discardableResult
    public func insertRow<C>(contentsOf newElements: __owned C, at i: RowIndex, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where C : Collection,C.Element==RowElement {
    ...
    @discardableResult
    public func replaceRow(at position: RowIndex, in s:SectionIndex, with newElement: RowElement, userInfo:[AnyHashable:Any]?=nil)->Difference {
        let elements = self.contentCollection[s].rda_elements
        return self.replaceRow(position..<elements.index(after: position), with: CollectionOfOne(newElement), in: s, userInfo: userInfo)
    }

    @discardableResult
    public func replaceRow<C:Collection, R:RangeExpression>(_ subrange: R, with newElements: __owned C, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where C.Element==RowElement, R.Bound==RowIndex {
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
    ...
    @discardableResult
    public func removeRow(at position: RowIndex, in s:SectionIndex,userInfo:[AnyHashable:Any]?=nil) -> Difference {
        let elements = self.contentCollection[s].rda_elements
        return self.replaceRow(position..<elements.index(after: position), with: EmptyCollection(), in: s, userInfo: userInfo)
    }

    @discardableResult
    public func removeRow<R:RangeExpression>(_ bounds: R, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where R.Bound==RowIndex {
    ...
    public func removeAllRow(userInfo:[AnyHashable:Any]?=nil, where shouldBeRemoved: (RowElement) -> Bool)->Difference {
        ...
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
        return self.removeAllRow(in: self.contentCollection.startIndex..<self.contentCollection.endIndex, userInfo: userInfo, where: shouldBeRemoved)
    }

    @discardableResult
    public func removeAllRow<R:RangeExpression>(in range:R, userInfo:[AnyHashable:Any]?=nil, where shouldBeRemoved: (RowElement) -> Bool)->Difference where R.Bound==SectionIndex{
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
    ...
    ///待测试
    @discardableResult
    public func moveRow(fromRow:RowIndex,fromSection:SectionIndex, toRow:RowIndex, toSection:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference {
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}
        ...
        let change = Difference.Change.elementMove(fromOffset: fromRow, fromSection: fromSection, toOffset: toRow, toSection: toSection, element: element as? RowElement)
