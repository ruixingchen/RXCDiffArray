...
public protocol SimpleSectionElementProtocol {

    ///这里必须要有setter
    var rsda_elements:[Any] {get set}

}

public protocol RXCSimpleDiffArrayDelegate: AnyObject {

    func simpleDiffArray<SectionContainer: RangeReplaceableCollection, RowElement>(array:RXCSimpleDiffArray<SectionContainer, RowElement>, didChange difference:RDADifference<SectionContainer.Element,RowElement>)
}
...
/// 范型表示Section的数据的容器类型, 一般是一个Array类型, 如RXCSimpleDiffArray<[Card]>, 兼容其他RangeReplaceableCollection类型
public final class RXCSimpleDiffArray<SectionContainer: RangeReplaceableCollection, RowElement> {
    ...
    public weak var delegate:RXCSimpleDiffArrayDelegate?
    ...
    internal func notifyDelegate(diff:RDADifference<SectionContainer.Element, RowElement>, userInfo:[AnyHashable:Any]?) {
        if userInfo?[RXCSimpleDiffArray.Key.notify] as? Bool ?? true {
            //only false works, anything else is *true*
            self.delegate?.simpleDiffArray(array: self, didChange: diff)
        }
    }
...
extension RXCSimpleDiffArray where SectionContainer.Index==Int, SectionContainer.Element: SimpleSectionElementProtocol {
    ...
    public func element(at indexPath:IndexPath)->RowElement? {
        ...
        if indexPath.item < 0 || indexPath.item >= section.rsda_elements.count {return nil}
        let element = section.rsda_elements[indexPath.item]
...
extension RXCSimpleDiffArray where Element: SimpleSectionElementProtocol, SectionContainer.Index==Int {
    ...
    @discardableResult
    public func rda_replaceSection<C:Collection, R:RangeExpression>(_ subrange: R, with newElements: __owned C, userInfo:[AnyHashable:Any]?=nil)->Difference where C.Element==RXCSimpleDiffArray.Element, R.Bound==SectionContainer.Index {
...
//这里要求Int是为了兼容RDADifference
extension RXCSimpleDiffArray where Element: SimpleSectionElementProtocol, SectionIndex==Int {
    ...
    public func rda_appendRow<S>(contentsOf newElements: __owned S, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where S : Collection, S.Element==RowElement {
        ...
        let elements = self.contentCollection[s].rsda_elements
    ...
    @discardableResult
    public func rda_replaceRow<C:Collection, R:RangeExpression>(_ subrange: R, with newElements: __owned C, in s:SectionIndex, userInfo:[AnyHashable:Any]?=nil)->Difference where C.Element==RowElement, R.Bound==RowIndex {
        let safe:Bool = self.threadSafe
        if safe {self.lockContent()}
        defer {if safe {self.unlockContent()}}

        var section = self.contentCollection[s]
        var elements = section.rsda_elements
        ...
        section.rsda_elements = elements
...
//MARK: - Collection
extension RXCSimpleDiffArray: Collection {
...
extension RXCSimpleDiffArray where SectionContainer.Element: Differentiable, RowElement: Differentiable {
    ...
    func batchWithDifferenceKit(batch:()->Void)->[Difference] {
        ...
        let oldElements = self.contentCollection.map {
            return ArraySection(model: $0, elements: $0.rsda_elements)
        }
        batch()
