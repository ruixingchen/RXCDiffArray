...
///delete generic type constraint
public struct SimpleArraySection<Model, Element>: DifferentiableSection {
    ...
    public var elements: [Differentiable] {
        get {
            return self._elements as! [Differentiable]
        }
        set {
            self._elements = newValue as! [Element]
        }
    ...
    public var differenceIdentifier: AnyHashable {
        return (model as! Differentiable).differenceIdentifier
    }
    ...
    @inlinable
    public init<C>(source: DifferentiableSection, elements: C) where C : Collection, C.Element == Differentiable {
        guard let s = source as? SimpleArraySection<Differentiable, Differentiable> else {
            fatalError("invalid source type")
        }
        self.init(model: s.model as! Model, elements: elements.map({$0 as! Element}))
    }
    ...
    @inlinable
    public func isContentEqual(to source: Any) -> Bool {
        if let s = source as? SimpleArraySection<Model, Element> {
            return (model as! Differentiable).isContentEqual(to: s.model)
        }
        return false
    }
}
